...
HOG (histogram of oriented gradient) là một thuật toán tạo ra các bộ mô tả đặc trưng (feature descriptor) được sử dụng phổ biến trong computer vision nhằm mục đích **phát hiện vật thể** (object detection). Thuật toán hoạt động dựa trên nguyên lý thống kê phân phối histogram của độ lớn gradient (gradient magnitute) theo các nhóm bins của phương gradient (gradient orientation). Các véc tơ HOG cục bộ được tạo thành trong quá trình lấy đạo hàm thông qua phép tích chập một kernel filter với các vùng cục bộ (local portion) của hình ảnh. Để xác định các vùng cụ bộ, một lưới ô vuông được tạo thành, chia hình ảnh thành các ô vuông bằng nhau. Véc tơ HOG sau khi được tính toán trên một vùng cục bộ sẽ sử dụng một chuẩn hóa cục bộ (local normalization) để bất biết với các phép scale cường độ sáng của ảnh và đồng thời giúp cải thiện độ chính xác của việc phát hiện vật thể. 

Trên đây là toàn bộ lý giải vắn tắt về nguyên lý hoạt động của HOG. Có thể sẽ hơi khó hiểu với bạn đọc lúc đầu nhưng đừng lo lắng. Chúng ta sẽ sáng tỏ sau khi đọc tiếp các chương sau. Vậy HOG có những ứng dụng cụ thể như thế nào? Một số tác vụ áp dụng có thể kể đến đã áp dụng HOG và mang lại độ chuẩn xác cao có thể kể đến là:
...
* **Tạo feature cho các bài toán phân loại ảnh**: Trong nhiều bài toán phân loại ảnh được thực hiện trên một bộ dữ liệu kích thước nhỏ thì sử dụng các mạng học sâu chưa chắc đã mang lại hiệu quả và dễ dẫn tới overfiting do dữ liệu ít thường không đủ để huấn luyện cho máy tính nhận tốt các đặc trưng chưng. Khi đó sử dụng HOG như một bộ tạo đặc trưng sẽ mang lại kết quả tốt hơn. Như vậy không hẳn lúc nào mạng học sâu cũng mang lại kết quả khả quan mà đôi khi những phương pháp cổ điển vẫn rất hữu hiệu.

Tiếp theo chúng ta sẽ đi tìm hiểu chi tiết thuật toán HOG. Để thuận tiện mình sẽ lý giải trước các thuật ngữ được sử dụng:

* **Feature Descriptor**: Bộ mô tả đặc trưng, là kết quả của một phép biến đổi input thông qua một phương pháp biến đổi nào đó như HOG, SUFT để nhằm tạo ra các features.
...
* **Local normalization**: Phép chuẩn hóa được thực hiện trên một vùng cục bộ. Thường là chia cho norm chuẩn bậc 2 hoặc norm chuẩn bậc 1. Mục đích của việc chuẩn hóa là để đồng nhất độ sáng của toàn bộ các vùng cục bộ. Ta sẽ làm rõ hơn trong phần trình bày thuật toán.
